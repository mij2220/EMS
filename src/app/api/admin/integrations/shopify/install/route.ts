...
export async function GET(req: NextRequest) {
  ...
  if (!clientId || !secret) {
    return NextResponse.json(
      { error: "SHOPIFY_CLIENT_ID (and JWT_SECRET) must be set on the server before connecting Shopify." },
      { status: 500 }
    );
  }
  ...
  const redirectUri = new URL("/api/admin/integrations/shopify/callback", req.nextUrl.origin).toString();
