...
type Employee = { id: string; name: string; role: string | null; lastPaymentDate: string | null; lastPaymentAmount: number | null; advanceBalance: number };
    ...
    <AppShell active="employees" title="Employees" desc="Employee master, salary history and payroll" tenantName={tenantName} userInitial={userInitial}>
      ...
      <div className="mockup-card !p-0 overflow-hidden">
        <table className="w-full text-sm">
          ...
          <tbody>
            {employees.map((e) => (
              <tr key={e.id} onClick={() => router.push(`/dashboard/employees/${e.id}`)} className="cursor-pointer hover:bg-slate-50" style={{ borderTop: "1px solid var(--line)" }}>
